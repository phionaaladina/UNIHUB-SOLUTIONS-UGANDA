...
const AdminDiscountsPage = () => {
  ...
  const fetchDiscounts = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/v1/discounts/?${params.toString()}`, {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  }, [fetchDiscounts, loggedInUserRole]);
  ...
  const handleSaveDiscount = async (e) => {
    ...
    try {
      let url = `${API_BASE_URL}/api/v1discounts/`;
      let method = 'POST';

      if (isEditing) {
        url = `${API_BASE_URL}/api/v1/discounts/${selectedDiscount.discount_id}`;
        method = 'PUT';
      }

      const payload = {
        code: selectedDiscount.code,
        percentage: parseFloat(selectedDiscount.percentage),
        is_active: selectedDiscount.is_active,
        start_date: selectedDiscount.start_date ? new Date(selectedDiscount.start_date).toISOString() : null,
        end_date: selectedDiscount.end_date ? new Date(selectedDiscount.end_date).toISOString() : null,
      };
  ...
  const handleDeleteDiscount = async (discountId) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/v1/discounts/${discountId}`, {
        method: 'DELETE',
  ...
  if (loggedInUserRole !== 'admin' && loggedInUserRole !== 'super_admin') {
    return <Alert variant="warning" className="m-4">Access Denied</Alert>;
  }
            ...
            <tbody>
              {discounts.map((discount) => (
                <tr key={discount.discount_id}>
                  ...
                  <td>
                    <div className="d-flex gap-2">
                      <Button variant="warning" size="sm" onClick={() => handleEditDiscount(discount)}>Edit</Button>
                      <Button variant="danger" size="sm" onClick={() => handleDeleteDiscount(discount.discount_id)}>Delete</Button>
                    </div>
                  </td>
                </tr>
              ))}
